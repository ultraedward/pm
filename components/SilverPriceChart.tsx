...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    ...
    >
      <p className="text-gray-500 text-xs mb-1">{new Date(label).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</p>
      <p className="font-black tabular-nums text-white">{fmt(payload[0].value)}<span className="text-gray-500 font-normal"> /ozt</span></p>
    </div>
  );
}
...
export function SilverPriceChart() {
  ...
  const chartData = data.map((d) => ({ t: d.t, price: d.price }));
    ...
    <div className="space-y-4">
      ...
      {!loading && data.length > 1 && change != null && pct != null && high && low && (
        <div className="grid grid-cols-4 gap-3">
          {[
            { label: `${range} change`,  val: `${up ? "+" : ""}${fmt(change)}`, sub: `${up ? "+" : ""}${pct.toFixed(2)}%`, accent: up },
